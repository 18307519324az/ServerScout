...
import type {
  ...
  AiBriefingResult, RiskScoreDetail, SystemMode,
} from '../types'
...
export const fetchSystemMode = () =>
  http.get<ApiResponse<SystemMode>>('/v1/system/mode')
...
// Risk Scores
export const fetchRiskScoresByTask = (taskId: number | string) =>
  http.get<ApiResponse<RiskScoreDetail[]>>(`/v1/risk-scores/task/${taskId}`)

export const fetchRiskScoresByAsset = (assetId: number | string) =>
  http.get<ApiResponse<RiskScoreDetail[]>>(`/v1/risk-scores/asset/${assetId}`)

export const fetchTopRiskScores = (limit: number = 10) =>
  http.get<ApiResponse<RiskScoreDetail[]>>('/v1/risk-scores/top', { params: { limit } })

export const recalculateRiskScores = (taskId: number | string) =>
  http.post<ApiResponse<{ taskId: number; scored: number }>>(`/v1/risk-scores/task/${taskId}/recalculate`)
