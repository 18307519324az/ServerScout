...
public class DemoScannerStrategy implements ScannerStrategy {
    ...
    public ScanResult execute(ScanTask task) {
        ...
        // Build target IPs
        List<String> targetIps = deriveIps(task.getTargetRange(), "FULL".equalsIgnoreCase(scanType) ? 5 : 3);
        ...
        boolean isFull = "FULL".equalsIgnoreCase(scanType);
        if (isFull) {
    ...
    private List<ScanResult.VulnEntry> generateVulns(List<String> targetIps, String scanType) {
        ...
        boolean isFull = "FULL".equalsIgnoreCase(scanType) || "NUCLEI".equalsIgnoreCase(scanType);
