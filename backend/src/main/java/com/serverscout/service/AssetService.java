...
public class AssetService {
    ...
    public void deleteAsset(Long id) {
        ...
        // Delete all child records referencing this asset or its ports
        List<Port> ports = portRepository.findByAssetId(id);
        List<Long> portIds = ports.stream().map(Port::getId).toList();

        if (!portIds.isEmpty()) {
            crawledUrlRepository.deleteByPortIdIn(portIds);
        }
        crawledUrlRepository.deleteByAssetId(id);
        ...
        riskScoreRepository.deleteByAssetId(id);
        honeypotDetectionRepository.deleteByAssetId(id);
