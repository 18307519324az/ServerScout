      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border dark:border-gray-700 shadow-sm p-4 mb-6">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-bold text-lg dark:text-white">爬虫发现</h2>
        ...
        {isLoading ? (
          <div className="flex items-center justify-center py-8 text-gray-400">
            ...
            <span className="text-sm">加载爬虫数据...</span>
          </div>
        ) : urls.length === 0 ? (
          /* Empty state — always visible */
          <div className="text-center py-8">
            <Globe className="w-10 h-10 mx-auto mb-2 text-gray-300 dark:text-gray-600" />
            <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">暂无爬虫数据</p>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-1 max-w-md mx-auto">
              运行一次<strong>带 Web 指纹识别</strong>的扫描任务，系统将自动爬取 Web 服务页面并截取网站截图
            </p>
            <div className="mt-3 flex items-center justify-center gap-4 text-[10px] text-gray-400 dark:text-gray-500">
              <span className="flex items-center gap-1"><Camera className="w-3 h-3" /> 自动截图</span>
              <span className="flex items-center gap-1"><Globe className="w-3 h-3" /> 页面爬取</span>
              <span className="flex items-center gap-1"><FileText className="w-3 h-3" /> 内容提取</span>
            </div>
          </div>
        ) : (
          <>
            ...
            {screenshots.length > 0 && (
              <div className="mb-4">
                <h3 className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-2 flex items-center gap-1">
                  <Camera className="w-3.5 h-3.5" /> 页面截图
                </h3>
                      ...
                      {expanded.has(u.id) && (
                        <div className="px-3 pb-3 border-t dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                          <div className="grid grid-cols-2 gap-x-4 gap-y-1 mt-2 text-xs">
                            ...
                            <div>
                              <span className="text-gray-400">发现链接: </span>
                          ...
                          {u.bodyText && (
                            <div className="mt-2">
                              <p className="text-[10px] text-gray-400 mb-1 flex items-center gap-1">
                                <FileText className="w-3 h-3" /> 页面文本摘要
                              </p>
